...
export default {
  ...
  setup() {
    ...
    onMounted(()=>{
      myMap = leaflet.map('mapid').setView([13.005873, 5.247552], 9);
    ...
    const getIpInfo = async () => {
      try {
        ...
        const clientIpResult = clientIp.data
        let data
          if(!queryIp.value) {
            data = await axios.get(`https://geo.ipify.org/api/v2/country,city?apiKey=at_FHmifZsdVxhJVx5It5qODuhy9QvyB&ipAddress=${clientIpResult.ip}`)
          }else{
            data = await axios.get(`https://geo.ipify.org/api/v2/country,city?apiKey=at_FHmifZsdVxhJVx5It5qODuhy9QvyB&ipAddress=${queryIp.value}`)
          }
